...
# Install dev dependencies with verbose output for debugging
RUN pip install --no-cache-dir --verbose -r requirements-dev.txt
...
RUN find backend/ -name "*.py" -exec chmod 644 {} \;
RUN chmod +x backend/run.py
